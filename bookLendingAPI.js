var bookSystem = function () {
	...
	function privateAddBook() {
		var newBook = prompt('\n Please add book in format: "Name, Author, ISBN" separating with commas as shown.').trim();
		if (newBook && newBook.includes(',')) {
			newBook = newBook.split(',');
			if (!isNaN(newBook[2]) || newBook[2].length === 10 || newBook[2].length === 13) {
				newBook = ['Index: ' + function () {
					return privateStore.length + 1;
				}(), ' Name: ' + newBook[0], ' Author: ' + newBook[1], ' ISBN: ' + newBook[2] + '\n'];
				privateStore.push(newBook);
				console.log('Successfully added a book to store.');
				return true;
			} else {
	...
	function displayAvailableBooks() {
		if (privateStore.length === 0) {
			...
			console.log('Here\'s the list of available books: \n' + privateStore);
			return true;
		}
	}
	...
	function interfaceInner() {
		...
		if (end) {
			if (action === 'a' || action === 'b' || action === 'c' || action === 'd' || action === 'e') {
				console.log('Thank you for stopping by today.');
			} else {
